...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Экспортируем switchTab сразу — до того как admin.js может его перехватить
...
function updateTankSVG(type, stored, capacity) {
    ...
    if (type === 'oil') {
        const pctEl = document.getElementById('oilBarrelPct');
        const valEl = document.getElementById('oilBarrelVal');
        if (pctEl) pctEl.textContent = pctRounded + '%';
        if (valEl) valEl.textContent = stored.toFixed(1) + ' барр.';
    } else {
        const pctEl = document.getElementById('gasBarrelPct');
        const valEl = document.getElementById('gasBarrelVal');
        if (pctEl) pctEl.textContent = pctRounded + '%';
        if (valEl) valEl.textContent = stored.toFixed(0) + ' м³';
    }
}
